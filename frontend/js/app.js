...
async function loadInfo() {
  ...
  const price = web3.utils.fromWei(runtimeConfig.publicMintPrice, 'ether');
...
function setTotalPrice() {
  ...
  const totalPriceWei = BigInt(runtimeConfig.publicMintPrice) * BigInt(mintInputValue);
...
async function mint() {
  ...
  const value = BigInt(runtimeConfig.publicMintPrice) * BigInt(amount);
  const publicMintActive = await contract.methods.mintingActive().call();
  const presaleMintActive = await contract.methods.presaleActive().call();
